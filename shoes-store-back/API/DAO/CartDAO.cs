...
namespace API.DAO
{
    public class CartDAO
    {
        ...
        public ResponseMessage AddCartItem(int accountID, CartItemDTO itemCart)
        {
            try
            {
                ...
                CartItem addItem = new CartItem
                {
                    ...
                    Variant = variant,
                    TotalItemPrice = (double)variant.Product.ProductPrice * itemCart.Quantity
                };
        ...
        public void TotalPriceCart(int accountID)
        {
            ...
            if (getCart != null)
            {
                var getCartItem = db.CartItem.Include(x => x.Cart)
                                     ...
                                     .ToList();
                double totalPrice = getCartItem.Sum(x => x.TotalItemPrice);
        ...
        public void TotalItemPrice(int variantID)
        {
            ...
            getCartItem.TotalItemPrice = (double)getCartItem.Variant.Product.ProductPrice * getCartItem.Quantity;
            db.CartItem.Update(getCartItem);
        }
    }
}
